...
import src.utils.{Array2Vector, DoubleUDF, Vector2DoubleUDF}

object MainTask3PipelineBis
{
  ...
  def main(args: Array[String]){
    ...
    val labelShifter = new DoubleUDF(x => x-1922).setInputCol("labelV").setOutputCol("label")

    val pipeline = new Pipeline()
      .setStages(Array(regexTokenizer, arr2Vect, labelSlicer, vectorDisassembler, labelShifter, featureSlicer))
    ...
    val pipelineLa = new Pipeline().setStages(Array(learningAlgorithm))
    // fit on the training data
    val pipelineModelLa = pipelineLa.fit(train)
    // get model summary and print RMSE
    val modelSummary = pipelineModelLa.stages(0).asInstanceOf[LinearRegressionModel]
    ...
    val predictions = pipelineModelLa.transform(test)
    //print predictions
    predictions.show(5)
  }
