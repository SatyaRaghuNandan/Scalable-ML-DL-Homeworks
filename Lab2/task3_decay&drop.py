...
y1 = tf.nn.relu(y1_logits)
y1d = tf.nn.dropout(y1, keeping_probability)
y2_logits = tf.matmul(y1d, w2) + b2
y2 = tf.nn.relu(y2_logits)
y2d = tf.nn.dropout(y2, keeping_probability)
y3_logits = tf.matmul(y2, w3) + b3
y3 = tf.nn.relu(y3_logits)
y3d = tf.nn.dropout(y3, keeping_probability)
y4_logits = tf.matmul(y3, w4) + b4
y4 = tf.nn.relu(y4_logits)
y4d = tf.nn.dropout(y4, keeping_probability)
y5_logits = tf.matmul(y4, w5) + b5
y5 = tf.nn.softmax(y5_logits)

# 3. Define the loss function  
cross_entropy = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=y_, logits=y5_logits))
...
plt.title("Loss zoom")
plt.plot(x_range, train_c[zoom_point:], label="train")
plt.plot(x_range, test_c[zoom_point:], label="test")
plt.grid(True)
plt.show()
